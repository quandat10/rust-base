...
pub fn generate_jwt(
    ...
    let claims = TokenClaims {
        sub: token_details.account_code.to_string(),
    ...
    let token = jsonwebtoken::encode(
        &header,
        &claims,
        &jsonwebtoken::EncodingKey::from_rsa_pem(decoded_private_key.as_bytes()).unwrap(),
    )?;
...
pub fn verify_jwt(
    ...
    let account_code = Uuid::parse_str(decoded.claims.sub.as_str()).unwrap();
